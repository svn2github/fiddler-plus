...
namespace FiddlerControls
{
    public partial class Fonts : UserControl
    {
        ...
        private void drawitem(object sender, DrawListViewItemEventArgs e)
        {
            ...
            string symbol;
            if (i < 256) // Даже для юникода первые 256 символов отображаем в кодировки win-1251 (т.к. клиент часто выводит ASCII текст юникодными шрифтами) 
                symbol = System.Text.ASCIIEncoding.GetEncoding(1251).GetString(new byte[] { (byte)i });
            else // Для всех остальных юникодных символов отображаем юникодные символы
                symbol = c.ToString();

            e.Graphics.DrawString(symbol, Fonts.DefaultFont, Brushes.Gray, e.Bounds.X + e.Bounds.Width / 2, e.Bounds.Y + e.Bounds.Height / 2);
            if (bmp != null)
            {
                int width = bmp.Width;
        ...
        private void onSelectChar(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (i < 256) // Даже для юникода первые 256 символов отображаем в кодировки win-1251 (т.к. клиент часто выводит ASCII текст юникодными шрифтами) 
                    ...
                    symbol = ((char)i).ToString();

                if ((int)treeView.SelectedNode.Parent.Tag == 1) // Unicode
                    toolStripStatusLabel1.Text = String.Format("'{0}' : {1} (0x{1:X}) XOffset: {2} YOffset: {3}", symbol, i, UnicodeFonts.Fonts[(int)treeView.SelectedNode.Tag].Chars[i].XOffset, UnicodeFonts.Fonts[(int)treeView.SelectedNode.Tag].Chars[i].YOffset);
                else
                    toolStripStatusLabel1.Text = String.Format("'{0}' : {1} (0x{1:X})", symbol, i);
            }
        }

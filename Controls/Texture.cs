...
namespace FiddlerControls
{
    public partial class Texture : UserControl
    {
        ...
        private void OnClickExportAll(object sender, EventArgs e)
        {
            string path = Path.Combine(AppDomain.CurrentDomain.SetupInformation.ApplicationBase, "texmaps");
            ...
            {
                int i = (int)item.Tag;
                string FileName = Path.Combine(path, String.Format("T0x{0:X4}.bmp", i));
            ...
            MessageBox.Show(
                String.Format("Все текстуры сохранены в {0}", path),
                "Сохранено",

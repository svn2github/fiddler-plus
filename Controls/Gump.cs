...
namespace FiddlerControls
{
    public partial class Gump : UserControl
    {
            ...
            using (FolderBrowserDialog dialog = new FolderBrowserDialog())
            {
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    for (int i = 0; i < listBox.Items.Count; ++i)
                    {
                        ...
                        if (index >= 0)
                        {
                            string FileName = Path.Combine(dialog.SelectedPath, String.Format("G0x{0:X4}.bmp", index));
        ...
        private void OnClick_ImportAllBmp(object sender, EventArgs e)
        {
            using (FolderBrowserDialog dialog = new FolderBrowserDialog())
            {
                dialog.SelectedPath = "I:\\@@@@";
                dialog.Description = "Выберите папку c файлами в формате \"G0x????.bmp\" или \"G?????.bmp\".";
                dialog.ShowNewFolderButton = false;
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    string[] hexFiles = System.IO.Directory.GetFiles(dialog.SelectedPath, "G0x????.bmp", SearchOption.TopDirectoryOnly);
                    string[] decFiles = System.IO.Directory.GetFiles(dialog.SelectedPath, "G?????.bmp", SearchOption.TopDirectoryOnly);
                    ...
                    for (ushort index = 0; index < 0xFFFF; ++index)
                    {
                        if (!files.ContainsKey(index))
                            continue;
                        ...
                        if (useCKeyToolStripMenuItem.Checked)
                            import = FiddlerControls.Utils.CKeyFilter(import);
                        if (useWColToolStripMenuItem.Checked)
                            import = FiddlerControls.Utils.WColFilter(import, false);
                        if (useBColToolStripMenuItem.Checked)
                    ...
                    //if (Ultima.Gumps.)
                    {
                        ...
                        MessageBox.Show(String.Format("Гампы были импортированы и сохранены в {0}", AppDomain.CurrentDomain.SetupInformation.ApplicationBase), "Сохраннено", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
                    }

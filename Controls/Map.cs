...
namespace FiddlerControls
{
    public partial class Map : UserControl
    {
        ...
        #region Facet
        private enum ViewModType
        {
            Map,
            Facet
        }

        private static ViewModType ViewMod = ViewModType.Map;

        private Ultima.Facet Facet = null;

        private void OnClickSwitchFacetMap(object sender, EventArgs e)
        {
            if(ViewMod == ViewModType.Facet)
            {
                ViewMod = ViewModType.Map;
                SwitchFacetMap.Text = "Режим просмотра: Map";
                PreloadMap.Enabled = true;
                showStaticsToolStripMenuItem1.Enabled = true;
            }
            else if(ViewMod == ViewModType.Map)
            {
                ViewMod = ViewModType.Facet;
                SwitchFacetMap.Text = "Режим просмотра: Facet";
                PreloadMap.Enabled = false;
                showStaticsToolStripMenuItem1.Enabled = false;
                ChangeFacet();
            }
        }
        ...
        private void GenerateFacet(Ultima.Map map, Ultima.FacetGenAlgorithm alg)
        {
            Cursor.Current = Cursors.WaitCursor;
            Ultima.Facet facet = new Ultima.Facet(map, alg);
        ...
        private void OnClickAlgUltimaMapToolStripMenuItem(object sender, EventArgs e)
        {
            GenerateFacet(FacetGenAlgorithm.UltimaMap);
        }

        private void OnClickAlgSimpleToolStripMenuItem(object sender, EventArgs e)
        {
            GenerateFacet(FacetGenAlgorithm.Simple);
        }

        private void OnClickAlgAltMaskToolStripMenuItem(object sender, EventArgs e)
        {
            GenerateFacet(FacetGenAlgorithm.AltMask);
        }

        private void OnClickAlgAltMaskNoiseToolStripMenuItem(object sender, EventArgs e)
        {
            GenerateFacet(FacetGenAlgorithm.AltMaskNoise);
        }
        ...
        private void OnClickFacetFromBmpToolStripMenuItem(object sender, EventArgs e)
        {
        ...
        {
            int y = Ultima.Map.Trammel.Width;
            Cursor.Current = Cursors.WaitCursor;
            LoadMapOverlays();
            Options.LoadedUltimaClass["Map"] = true;
            Options.LoadedUltimaClass["RadarColor"] = true;
            
            currmap = Ultima.Map.Sosaria;
			//currmap = Ultima.Map.Britania;
            ...
            ChangeMapNames();
            ZoomLabel.Text = String.Format("Масштаб: {0}%", 100 * Zoom);
            SetScrollBarValues();
            Refresh();
            pictureBox.Invalidate();
            //OnClickSwitchFacetMap(this, new EventArgs());   // Переключаем на показ Facet по умолчанию
            if (currmap == Ultima.Map.Sosaria && Ultima.Map.Sosaria.Width == 12288) {        // Скролим к форту Бронби при открытии карты Соссарии
                hScrollBar.Value = (int)Math.Max(0, 7215 - pictureBox.Right / Zoom / 2);
                vScrollBar.Value = (int)Math.Max(0, 3115 - pictureBox.Bottom / Zoom / 2);
            }
            Cursor.Current = Cursors.Default;
            if (!Loaded)
            {
        ...
        private void OnMouseMove(object sender, MouseEventArgs e)
        {
            ...
            CoordsLabel.Text = String.Format("Координаты: {0},{1}", xDelta, yDelta);
            if (moving)
            {
        ...
        private void onClick_GotoClientLoc(object sender, EventArgs e)
        {
            ...
            pictureBox.Invalidate();
            ClientLocLabel.Text = String.Format("Координаты в клиенте: {0},{1},{2},{3}", x, y, z, Options.MapNames[mapClient]);
        }
        ...
        private void OnOpenContext(object sender, CancelEventArgs e)  // Speichern fьr GetMapInfo
        {
            ...
            if (Ultima.Map.Trammel.Width != 12288)
            {
                ...
                malasToolStripMenuItem.Visible  = true;
                tokunoToolStripMenuItem.Visible = true;
                terMurToolStripMenuItem.Visible = true;
            }
            else
            {
                malasToolStripMenuItem.Visible  = false;
                tokunoToolStripMenuItem.Visible = false;
                terMurToolStripMenuItem.Visible = false;
            }
        }
        ...
        private void OnPaint(object sender, PaintEventArgs e)
        {
            ...
            else if(ViewMod == ViewModType.Facet)
            {
                int r_width = (int)(e.ClipRectangle.Width / Zoom);
                int r_height = (int)(e.ClipRectangle.Height / Zoom);
                int r_x = Math.Max(0, hScrollBar.Value);
                int r_y = Math.Max(0, vScrollBar.Value);
                if (r_x + r_width >= Facet.Width)
                    r_x = Facet.Width - 2 - r_width;
                if (r_y + r_height >= Facet.Height)
                    r_y = Facet.Height - 2 - r_height;
                map = Facet.GetImage(r_x, r_y, r_width, r_height);
        ...
        #region Extract Map and Facet
        
        private void ExtractMapAs(ImageFormat format, string extention)
        {
            Cursor.Current = Cursors.WaitCursor;
            string path = AppDomain.CurrentDomain.SetupInformation.ApplicationBase;
            string name = String.Format("map{0}-{1}.{2}", currmapint, Options.MapNames[currmapint], extention);
        ...
        private void ExtractFacetBmp(object sender, EventArgs e)
        {
            ExtractFacetAs(ImageFormat.Bmp, "bmp");
        }

        private void ExtractFacetPng(object sender, EventArgs e)
        {
            ExtractFacetAs(ImageFormat.Png, "png");
        }

        private void ExtractFacetTiff(object sender, EventArgs e)
        {
            ExtractFacetAs(ImageFormat.Tiff, "tiff");
        }

        private void ExtractFacetJpg(object sender, EventArgs e)
        {
            ExtractFacetAs(ImageFormat.Jpeg, "jpg");
        }
        ...
        public static void SaveMapOverlays()
        {
            ...
            bool entries = false;
            for (int i = 0; i < (Ultima.Map.Trammel.Width != 12288 ? 5 : 3); ++i)
            {
        ...
        private MapCombine showformMapCombine = null;
        private void OnClickCombine(object sender, EventArgs e)
        {
            if ((showformMapCombine == null) || (showformMapCombine.IsDisposed))
            {
                showformMapCombine = new MapCombine(currmap);
                showformMapCombine.TopMost = true;
                showformMapCombine.Show();
            }
        }
            ...
            while ((line = ip.ReadLine()) != null)
            {
                ...
                try
                {
                    ...
                    else if (line.StartsWith("ID"))
                    {
                        ...
                        line = line.TrimStart(' ');
                        line = line.TrimEnd(' ');
                        newtile.m_ID = (ushort)Math.Min(Convert.ToUInt16(line), (uint)Art.StaticLength);
                    }
    ...
    public class OverlayObject
    {
        ...
        public virtual void DrawOnMap(Graphics g) { }
        public virtual void DrawOnFacet(Graphics g) { }
    ...
    public class OverlayCursor : OverlayObject
    {
        ...
        public override void DrawOnMap(Graphics g)
        {
        ...
        public override void DrawOnFacet(Graphics g)
        {
            int x = (int)((Loc.X - Map.Round(Map.HScrollBar)) * Map.Zoom);
            int y = (int)((Loc.Y - Map.Round(Map.VScrollBar)) * Map.Zoom);
            g.DrawLine(pen, x - 4, y, x + 4, y);

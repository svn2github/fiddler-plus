...
namespace FiddlerControls
{
    public partial class ItemShow : UserControl
    {
        ...
        private void OnLoad(object sender, EventArgs e)
        {
            ...
            {
                for (int i = 0; i < Art.StaticLength; ++i)
                {
        ...
        private void listView_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count == 1)
            {
                ...
                else
                {
                    ...
                    graphiclabel.Text = String.Format("Graphic: 0x{0:X5} ({0})", i);
                    UpdateDetail(i);
                }
            }
        }
        ...
        private void UpdateDetail(int id)
        {
            ...
            DetailTextBox.AppendText(String.Format("Graphic: 0x{0:X5}\n", id));
        ...
        {
            int index;
            if (Utils.ConvertStringToInt(InsertText.Text, out index, 0, Ultima.Art.StaticLength - 1))
            {
        ...
        private void onClickShowFreeSlots(object sender, EventArgs e)
        {
            ...
            {
                ListViewItem item;
                for (int j = 0; j < Art.StaticLength; ++j)
                {
            ...
            using (FolderBrowserDialog dialog = new FolderBrowserDialog())
            {
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    for (int i = 0; i < listView1.Items.Count; ++i)
                    {
                        ...
                        if (index >= 0)
                        {
                            string FileName = Path.Combine(dialog.SelectedPath, String.Format("I0x{0:X4}.bmp", index));

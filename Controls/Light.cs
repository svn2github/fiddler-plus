...
namespace FiddlerControls
{
    public partial class Light : UserControl
    {
        ...
        private unsafe Bitmap GetImage()
        {
            ...
            Bitmap bit = new Bitmap(pictureBox1.Width, pictureBox1.Height, PixelFormat.Format16bppRgb555);

            if (!iGPreviewToolStripMenuItem.Checked) {
                Bitmap lightimg = Ultima.Light.GetImage(Ultima.Light.GetLight((int)treeView1.SelectedNode.Tag));       
                using (Graphics g = Graphics.FromImage(bit)) {
                    g.FillRectangle(new SolidBrush(lightimg.GetPixel(0,0)), 0, 0, bit.Width, bit.Height);
                    if (borderToolStripMenuItem.Checked) {
                        int w = 2;
                        g.FillRectangle(Brushes.LimeGreen, (bit.Width - lightimg.Width) / 2 - w, (bit.Height - lightimg.Height) / 2 - w, lightimg.Width + 2 * w, lightimg.Height + 2 * w);
                    }
                    g.DrawImage(lightimg, (bit.Width - lightimg.Width) / 2, (bit.Height - lightimg.Height) / 2);
                }
                return bit;
            }
            ...
            if (light != null)
            {
                ...
                using (Graphics gr = Graphics.FromImage(bit))
                    if (borderToolStripMenuItem.Checked) {
                        int w = 2;
                        var lightimg = Ultima.Light.GetImage(Ultima.Light.GetLight((int)treeView1.SelectedNode.Tag));
                        gr.FillRectangle(Brushes.SteelBlue, lightstartX - w, lightstartY - w, lightimg.Width + 2 * w, w);
                        gr.FillRectangle(Brushes.SteelBlue, lightstartX - w, lightstartY - w, w, lightimg.Height + 2 * w);
                        gr.FillRectangle(Brushes.SteelBlue, lightendX + 1, lightstartY - w, w, lightimg.Height + 2 * w);
                        gr.FillRectangle(Brushes.SteelBlue, lightstartX - w, lightendY + 1, lightimg.Width + 2 * w, w);
                    }
            }
            return bit;
        }
        ...
        private void AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            var light = Ultima.Light.GetImage(Ultima.Light.GetLight((int)treeView1.SelectedNode.Tag));
            sizeToolStripStatusLabel.Text = String.Format("{0:D3}x{1:D3}", light.Width, light.Height);
        }
                ...
                using (OpenFileDialog dialog = new OpenFileDialog())
                {
                    ...
                    if (dialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (dialog.FileName.Contains(".bmp"))
                            bmp = Ultima.Light.GetLight(bmp); // Utils.ConvertBmp(bmp);
                        int i = (int)treeView1.SelectedNode.Tag;
        ...
        private void OnClickFixLight(object sender, EventArgs e)
        {
            int id = (int) treeView1.SelectedNode.Tag;
            var light = Ultima.Light.GetLight(id);
            Ultima.Light.FixLight(ref light);
            Ultima.Light.Replace(id, light);
            treeView1.Invalidate();
            AfterSelect(this, (TreeViewEventArgs)null);
            Options.ChangedUltimaClass["Light"] = true;
        }
        ...
        private void OnClickExportBmp(object sender, EventArgs e)
        {
            ...
            Ultima.Light.GetImage(Ultima.Light.GetLight(i)).Save(FileName, ImageFormat.Bmp);
            MessageBox.Show(
        ...
        private void OnClickExportTiff(object sender, EventArgs e)
        {
            ...
            Ultima.Light.GetImage(Ultima.Light.GetLight(i)).Save(FileName, ImageFormat.Tiff);
            MessageBox.Show(
                String.Format("Light saved to {0}", FileName),
                "Saved",
                MessageBoxButtons.OK,
        ...
        private void OnClickExportJpg(object sender, EventArgs e)
        {
            ...
            Ultima.Light.GetImage(Ultima.Light.GetLight(i)).Save(FileName, ImageFormat.Jpeg);
            MessageBox.Show(
                String.Format("Light saved to {0}", FileName),
                "Saved",
                MessageBoxButtons.OK,
        ...
        private void LightTileTextChanged(object sender, EventArgs e)
        {
            if (!Loaded)
                return;
            int index;
            if (Utils.ConvertStringToInt(LightTileText.Text, out index, 0, Ultima.Art.StaticLength))
            {

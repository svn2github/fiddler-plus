...
namespace UoFiddler
{
    public static class Options
    {
        ...
        public static void Save()
        {
            ...
            sr.AppendChild(elem);
            comment = dom.CreateComment("Использовать карты сервера Quintessence?");
            sr.AppendChild(comment);
            elem = dom.CreateElement("QuintMaps");
			elem.SetAttribute("active", Map.Dangeon.Width == 6144 && Map.Sosaria.Width == 12288 ? true.ToString() : false.ToString());
            sr.AppendChild(elem);
			comment = dom.CreateComment("Pre-Aplha Felucca map 1024x1024");
			sr.AppendChild(comment);
			elem = dom.CreateElement("PreAplhaMapSize");
			elem.SetAttribute("active", Map.Britania.Width == 1024 ? true.ToString() : false.ToString());
            sr.AppendChild(elem);
            comment = dom.CreateComment("NewMapSize Felucca/Trammel width 7168?");
            sr.AppendChild(comment);
            elem = dom.CreateElement("NewMapSize");
			elem.SetAttribute("active", Map.Felucca.Width == 7168 && Map.Trammel.Width == 7168 ? true.ToString() : false.ToString());
            sr.AppendChild(elem);
            ...
            comment = dom.CreateComment("Аккаунт и пароль для подключения к серверу");
            sr.AppendChild(comment);
            elem = dom.CreateElement("Server");
            elem.SetAttribute("account",  FiddlerControls.Options.Account);
            elem.SetAttribute("password", FiddlerControls.Options.Password);
            sr.AppendChild(elem);

            comment = dom.CreateComment("Настройки работы терминала Telnet");
            sr.AppendChild(comment);
            elem = dom.CreateElement("Telnet");
            elem.SetAttribute("active", FiddlerControls.Options.Telnet_active.ToString());
            elem.SetAttribute("conl",   FiddlerControls.Options.Telnet_conl.ToString());
            elem.SetAttribute("logr",   FiddlerControls.Options.Telnet_logr.ToString());
            sr.AppendChild(elem);
        ...
        private static void Load()
        {
            ...
            elem = (XmlElement)xOptions.SelectSingleNode("QuintMaps");
            if (elem != null)
            {
                if ( bool.Parse(elem.GetAttribute("active")))
                {
                    ...
                    Map.Trammel.Width = 12288;
                    Map.Trammel.Height = 8192;
                    Map.StartUpSetDiff(false);
                }
                else
                {
					elem = (XmlElement)xOptions.SelectSingleNode("PreAplhaMapSize");
					if (elem != null) {
						if (bool.Parse(elem.GetAttribute("active"))) {
            ...
            elem = (XmlElement)xOptions.SelectSingleNode("Server");
            if (elem != null)
            {
                FiddlerControls.Options.Account  = elem.GetAttribute("account");
                FiddlerControls.Options.Password = elem.GetAttribute("password");
            }

            elem = (XmlElement)xOptions.SelectSingleNode("Telnet");
            if (elem != null)
            {
                FiddlerControls.Options.Telnet_active = bool.Parse(elem.GetAttribute("active"));
                FiddlerControls.Options.Telnet_conl   = bool.Parse(elem.GetAttribute("conl"));
                FiddlerControls.Options.Telnet_logr   = int.Parse(elem.GetAttribute("logr"));
            }

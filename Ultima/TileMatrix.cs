...
namespace Ultima
{
    public sealed class TileMatrix
    {
        ...
        public HuedTile[][][] ReadStaticBlock(int x, int y, bool patch = true)
        {
            if (x < 0 || y < 0 || x >= BlockWidth || y >= BlockHeight)
                return EmptyStaticBlock;

            HuedTile[][][] tiles = ReadStaticBlock(x, y);
        ...
        public Tile[] ReadLandBlock(int x, int y, bool patch = true)
        {
            if (x < 0 || y < 0 || x >= BlockWidth || y >= BlockHeight)
                return InvalidLandBlock;

            Tile[] tiles = ReadLandBlock(x, y);
...
static List<int> listlen = new List<int>();
        ...
        private unsafe HuedTile[][][] ReadStaticBlock(int x, int y)
        {
            try
            {
                ...
                {
                    int count = length / (Art.IsPreAlpha ? 11 : 7);
                    ...
                    try
                    {
						...
						if (Art.IsPreAlpha) {
							for (int i = 0; i < count; ++i) {
								IntPtr ptr = new IntPtr((long)gc.AddrOfPinnedObject() + i * sizeof(OldStaticTile));	
								StaticTile cur = new StaticTile((OldStaticTile)Marshal.PtrToStructure(ptr, typeof(OldStaticTile)));
								lists[cur.m_X & 0x7][cur.m_Y & 0x7].Add((ushort)(Math.Min(cur.m_ID, Art.StaticLength)), cur.m_Hue, cur.m_Z);
							} 
						} else {
							for (int i = 0; i < count; ++i) {
	...
	public struct OldStaticTile
	{
		public uint   m_Unknown; // Serial ??
		public ushort m_ID;
		public byte	  m_X;
		public byte   m_Y;
		public sbyte  m_Z;
    ...
    public struct StaticTile
    {
		...
		public StaticTile(OldStaticTile oldstaticblock)
		{
			m_ID  = (ushort)oldstaticblock.m_ID;
			m_X   =   (byte)oldstaticblock.m_X;
			m_Y   =   (byte)oldstaticblock.m_Y;
			m_Z   =  (sbyte)oldstaticblock.m_Z;
			m_Hue = 0;
		}
    }

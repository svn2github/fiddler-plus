...
namespace Ultima
{
    ...
    {
        private static int[] m_Header; // Заголовки блоков
        private static byte[] m_Trash; // Непонятный мусор в конце для HS клиентов
                ...
                using (FileStream fs = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read))
                {
                    using (BinaryReader bin = new BinaryReader(fs))
                    {
                        unsafe
                        {
                            ...
                            sbyte[] fdata;
                            int length = 4 + 8 * (64 + 4);
                            m_Header = new int[bin.BaseStream.Length / length];
                            length *= m_Header.Length;
                            while (bin.BaseStream.Position != length)
                            {
                            ...
                            m_Trash = bin.ReadBytes((int)(bin.BaseStream.Length - bin.BaseStream.Position));
                        }
                    }
                }
            }
        ...
        public static void Save(string path)
        {
            ...
            using (FileStream fs = new FileStream(FileName, FileMode.Create, FileAccess.Write, FileShare.Write))
            {
                using (BinaryWriter bin = new BinaryWriter(fs))
                {
                    ...
                    if(m_Trash != null && m_Trash.Length > 0)
                        bin.Write(m_Trash);
                }
            }
        }

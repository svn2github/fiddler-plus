...
namespace Ultima
{
    ...
    public class Animations
    {
        ...
        /// </summary>
        /// <param name="body"></param>
        public static int Translate(int body)
        {
            int _body = body;
            Translate(ref _body);
            return _body;
        ...
        {
            int count = 2048;// 400 + ((m_FileIndex.Index.Length - 35000) / 175);
        ...
        public static bool IsAnimDefinied(int body)
        {
            var _body = body;
            Translate(ref _body);
            int fileType = BodyConverter.Convert(ref _body);
            return IsAnimDefinied(_body, fileType);
        }
    ...
    public sealed class BodyTable
    {
        ...
        public static void Save(string file)
        {
            ...
            for (int i = 0; i < 10000; ++i)
                if (m_Entries.ContainsKey(i)) {
                    var entry = m_Entries[i] as BodyTableEntry;
                    if (entry.OldID == entry.NewID && entry.NewHue <= 0) continue;
                    text += String.Format("{0} {{{1}}} {2}{3}", entry.NewID, entry.OldID, entry.NewHue, Environment.NewLine);
                }

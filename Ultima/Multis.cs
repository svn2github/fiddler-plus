...
namespace Ultima
{
    ...
    public sealed class MultiComponentList
    {
        ...
        public MultiComponentList(BinaryReader reader, int count)
        {
            ...
            {
                if (Art.IsUOHS)
                    m_SortedTiles[i].m_ItemID = (ushort)(reader.ReadUInt16() & 0xFFFF);
                else if (Art.IsUOSA)
                    m_SortedTiles[i].m_ItemID = (ushort)(reader.ReadUInt16() & 0x7FFF);
                else
                    m_SortedTiles[i].m_ItemID = (ushort)(reader.ReadUInt16() & 0x3FFF);
                ...
                if (Art.IsUOHS)
                    m_SortedTiles[i].m_Hue = (ushort)reader.ReadInt32();
                ...
                case Multis.ImportType.UOAB:
                    ...
                    using (BinaryReader reader = new BinaryReader(fs))
                    {
                        ...
                        for (; itemcount < count; ++itemcount)
                        {
                            ...
                            if(Art.IsUOHS)
                                m_SortedTiles[itemcount].m_Hue = reader.ReadUInt16(); // hue
                            else
        ...
        private void ConvertList()
        {
            ...
            for (int i = 0; i < m_SortedTiles.Length; ++i)
            {
                ...
                tiles[xOffset][yOffset].Add((ushort)(m_SortedTiles[i].m_ItemID), (sbyte)m_SortedTiles[i].m_OffsetZ, (sbyte)m_SortedTiles[i].m_Flags);
            }
